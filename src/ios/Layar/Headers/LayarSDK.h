...
/**
 * @mainpage Layar SDK Documentation v8.0
 *
 ...
 * 
 * The Layar SDK is built for ARM7+ architecture using iOS 7 SDK. Applications that use the SDK should also be built using iOS 7 SDK.
 * The iOS 7 SDK requirement also applies if you target devices running iOS 6.
 * Using the iOS 7 SDK in turn requires you to use XCode version 5 to build your application.
 *
 ...
 *
 * Any project using the Layar SDK must have the "Minimum OS Version" set to 6.0. It must also specify "Architectures" of armv7 and/or armv7s.
 *
 ...
 * @li In the "Build Phases" of your target, the "Link Binary with Libraries" option must also contain libsbjson-ios.a and libMPOAuthMobile.a from external libraries supplied with Layar SDK. Note: If your project already includes these libraries, you can skip this step.
 ...
 */
@property (nonatomic, readonly) id<LayarSDKDelegate> delegate;
